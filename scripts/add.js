		...
		qrCodeImg.onload = () => {
			...
			try {
				...
				let img_data = new ImageData(
					context.getImageData(0, 0, canv.width, canv.height).data,
					canv.width,
					canv.height
				);
...
function isLegalURL(url) {
	...
	let regex_work = new RegExp(
		"^https://work.weixin.qq.com/gm/([a-zA-Z0-9-_]+)$"
	);
...
function processCourseInput() {
	let checks = [
		processCourseTermInput(),
		processDepCodeInput(),
		processNumCodeInput(),
		processCourseNameInput(),
	];
	if (checks.some((e) => !e)) {
		return false;
	}
